...
void set_boundary_conditions(std::vector<std::vector<double>> &temperature,
                             double left = 0, double right = 1, double top = 0,
                             double bottom = 0);
...
int main() {
  ...
  //
  // Taking the input as 3 will run a unit test. User will have the choice to
  // see the comparison of dfferent numerical solutions and the anayltical
  // solution
  ///////////////////////////////////////////////////////////////////////////////
  ...
  std::cin >> input;
  if (input == 1) {
    std::cout << "Please specify constant temperature boundary conditiont the "
                 "four boundaries "
...
void set_boundary_conditions(std::vector<std::vector<double>> &temperature,
                             ...
                             double bottom) {
  const int nx = temperature.size();
  const int ny = temperature[0].size();
  ...
  while (num_iter < 5000) {
    ...
    for (int i = 1; i < (nx - 1); i++) {
      for (int j = 1; j < (ny - 1); j++) {
        temperature[i][j] =
            (0.25) * (temperature[i][j + 1] + temperature[i - 1][j] +
...
void eight_point_stencil(std::vector<std::vector<double>> &temperature) {
  ...
  while (num_iter < 5000) {
    ...
    for (int i = 1; i < (nx - 1); i++) {
      for (int j = 1; j < (ny - 1); j++) {
        temperature[i][j] =
            (1.0 / 8.0) * (temperature[i - 1][j + 1] + temperature[i][j + 1] +
                           temperature[i + 1][j + 1] + temperature[i - 1][j] +
                           temperature[i + 1][j] + temperature[i - 1][j - 1] +
                           temperature[i][j - 1] + temperature[i + 1][j - 1]);
      }
    }
    // std::cout<<"Grid after iteration "<<num_iter<<"is: \n \n";
    // print_grid(temperature);
...
bool unit_test(char choice) {
    ...
    for (int k = 0; k < nx; k++) {
      ...
      for (int l = 0; l < ny; l++) {
        temp_temperature[k][l] =
            dummy_temp * (sin(((n * M_PI) / dim_y) * y_values[l]));
        reference_temperature[l][k] =
            reference_temperature[l][k] + temp_temperature[k][l];
  ...
  //     // std::cout << "Computed: ";
  // }
  write_results(x_values, y_values, temperature, reference_temperature,
...
//
// For the case 1 and 2 (numerical solutions), three columns are written: X (X
// coordinates of the nodes), Y (Y coordinates of the nodes) and Numerical
// solution (the value of Temperature in K at that node)
//
// In the above case, the name of the output file is "results.csv" and is saved
// in the working directory.
//
// For the 3rd case (unit test), 5 columns are written:X (X coordinates of the
// nodes), Y (Y coordinates of the nodes), Numerical solution (the numerical
// value of Temperature in K at that node), Analytical solution(the exact value
// of Temperature in K at that node) and Error (absolute error between the
// analytical and the numerical solution)
//
// In this case, the name of the output file is "unit_test_results.csv" and is
...
void write_results(
  ...
  // std::cout<<"Size of reference solution:
  // "<<reference_temperature[0][0]<<std::endl;
  if (reference_temperature.size() > 1) {
    ...
    myfile << "X"
           ...
           << "Y"
           << ","
           << "Numerical Solution (in K)" << std::endl;
