        ...
        double x=(i+1)*hx;
          for (int j = 0; j < columns_of_b; j++)
          {
            double y=(j+1)*hy;
                    ...
                    //std::cout<<" b at "<<i-1<<" "<<j-1<<" is "<<source_function_value;

                    if ((i == 0) && (j == 0))   //top left corner
                        b[i][j] = source_function_value-(m_left)/(pow(hy,2))-(m_top)/(pow(hx,2));
                    
                    else if ((i == 0) && (j == columns_of_b)) // top right corner
...
else
{
    ...
    for (size_t i = 1; i <(m_nx - 1); i++) {
      for (size_t j = 1; j <(m_ny - 1); j++) {
        m_temperature_values[i][j] = (1.0/a_kk)*(b[i-1][j-1]-(1.0/pow(hy,2))*(m_temperature_values[i][j-1]+m_temperature_values[i][j+1])-(1.0/pow(hx,2))*(m_temperature_values[i+1][j]+m_temperature_values[i-1][j]));
      }
    }
    num_iter=num_iter+1;
